 ...
 *
 * Also: ISMObj DArray_SetMaxSize DArray_SetSize
 *
 * Examples:
 *	DArray<void*> foo;            // empty array of pointers
 *
 ----*/
template <class E> DArray<E>::DArray(int size)
{
 ...
 *	}
 *
 * Also: DArray_Grow DArray_DestructItems
 *
 ----*/	
template <class E> inline void DArray<E>::ConstructItems(int start, int last)
{
 ...
 *	}
 *
 * Also: DArray_Grow DArray_ConstructItems
 *
 ----*/	
template <class E> inline void DArray<E>::DestructItems(int start, int nitems) { }
 ...
 *
 ****/
template <class E> inline DArray<E>::~DArray()
{
	Empty();
	free(m_Data);
	m_Data = NULL;
 ...
 *
 * Also: DArray_SetData DArray_SetMaxSize
 *
 ----*/
template <class E> bool DArray<E>::Grow(int growto)
{
...
//				GFX_ERROR(("DArray_Grow: out of memory"), false);
				return false;
		   }
		else
		   {
...
template <class E> inline E* DArray<E>::GetData() const
	{ return m_Data; }

/*++++
 *
 * Name: DArray_SetMaxSize
 ...
 *
 * Also: DArray_SetData DArray_SetSize DArray_GetElemSize DArray_Grow
 *
 ----*/
template <class E> bool DArray<E>::SetMaxSize(int s)
{
...
template <class E> inline int	DArray<E>::GetMaxSize() const
	{ return m_MaxSize; }

template <class E> inline int	DArray<E>::GetElemSize() const
	{ return sizeof(E); }
 ...
 *
 * Also: DArray_SetData DArray_Grow DArray_SetMaxSize
 *
 * Examples:
 *	DArray<ISMCol4f> cols(256);   // room for 256 colors
 *	int ncols = cols.GetSize();     // will be zero
 *	ncols = cols.GetMaxSize();      // will be 256
 *	cols.SetSize(ncols);            // calls 256 Color4 constructors NOT
 *
 ----*/
template <class E> bool inline DArray<E>::SetSize(int s)
{
	assert(s >= 0);
	if (s > m_MaxSize)
		if (!Grow(s))
...
template <class E> inline int	DArray<E>::GetSize() const
	{ return m_Size; }
 ...
 *
 * Examples:
 *	DArray<ISMCol4f> cols(16);        // room for 16 colors
 ...
 *
 ----*/
template <class E> bool DArray<E>::SetAt(int i, E p)
{
 ...
 *				DISM arrays do not check the range of the index so your
 ...
 *
 ----*/
template <class E> inline E& DArray<E>::GetAt(int i) const
	{ return m_Data[i]; }

template <class E> inline const E& DArray<E>::operator[](int i) const
	{ return m_Data[i]; }

template <class E> inline E& DArray<E>::operator[](int i)
	{ return m_Data[i]; }
 ...
 *
 * Examples:
 *	DArray<int> foo;   // define integer array
 ...
 *
 ----*/
template <class E> int DArray<E>::Find(const E& elem) const
{
	const E* p = m_Data;
 ...
 *
 * Also: DArray_SetAt DArray_SetSize DArray_RemoveAt
 *
 * Examples:
 *	DArray<float>	vals;
 ...
 *
 ----*/
template <class E> int inline DArray<E>::Append(const E& v)
{
	int	index = m_Size;
 ...
 *
 * Also: DArray_Append DArray_SetAt DArray_SetSize
 *
 * Examples:
 *	DArray<int16> zot(8); // room for 8 shorts
 ...
 *
 ----*/
template <class E> bool DArray<E>::RemoveAt(int i, int n)
{
	E*	elem;
	int	shuffle;
 ...
 *
 * Also: DArray_Append DArray_SetAt
 *
 * Examples:
 *	int	zap[3] = { 1, 2, 3 };
 *	DArray<int> zip(3, &zap);  // user managed
 *	DArray<int> zot;           // dynamic
 *	zot.Append(zip);               // adds 1 2 3
 *	zot.Append(zip);               // adds 1 2 3 again
 *
 ----*/
template <class E> int DArray<E>::Append(const DArray<E>& src)
{
 ...
 *
 * Also: DArray_SetSize DArray_IsEmpty
 *
 ----*/
template <class E> void DArray<E>::Empty()
{
 ...
 *
 * Also: DArray_SetSize DArray_Empty
 *
 ----*/
template <class E> inline bool DArray<E>::IsEmpty() const
	{ return m_Size == 0; }
